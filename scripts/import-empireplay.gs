 ...
 *  2. Ferramentas → Editor de Apps Script → cole este arquivo.
 *  3. Preencha SUPABASE_URL e SUPABASE_SERVICE_KEY abaixo.
 *  4. Execute a função `importarTudo()` ou execute cada função separadamente.
 *  5. Autorize as permissões quando solicitado.
 */

// ─── CONFIGURAÇÃO ────────────────────────────────────────────────────────────
const SUPABASE_URL      = 'https://rqwprnvlrobabfotmmnf.supabase.co';
const SUPABASE_SERVICE_KEY = PropertiesService.getScriptProperties().getProperty('SUPABASE_SERVICE_KEY') || 'COLE_AQUI_A_SERVICE_ROLE_KEY';
const SHEET_ID          = '1XYa6Pzd-lou3fzqaZgjhBYNb3Je2PB9Slu7ozzOghUo';
// ─────────────────────────────────────────────────────────────────────────────

/** Ponto de entrada único — executa todos os importadores em sequência. */
...
/** Retorna os dados de uma aba como array de objetos {coluna: valor}. */
...
/** Envia um array de registros para uma tabela Supabase via upsert. */
function upsertSupabase(tabela, registros) {
  if (!registros.length) { Logger.log('⚠️  Sem registros para ' + tabela); return; }

  // Supabase aceita até 500 linhas por requisição; faz lotes se necessário.
  const BATCH = 500;
...
/** Remove campos nulos/vazios para não sobrescrever dados existentes. */
function limpar(obj) {
  const out = {};
...
function importarTop50Spotify() {
  ...
  const registros = rows.map((r, i) => limpar({
    posicao           : Number(r['Posição'] || r['posicao'] || r['#'] || i + 1),
...
function importarComentariosMusicas() {
  ...
  const registros = rows.map(r => limpar({
    telegram_topic_id : Number(r['ID do tópico'] || r['telegram_topic_id'] || 0) || null,
    id_jogador        : String(r['ID do jogador'] || r['id_jogador'] || ''),
...
function importarComentariosVideos() {
  ...
  const registros = rows.map(r => limpar({
    telegram_topic_id : Number(r['telegram_topic_id'] || r['ID do tópico'] || 0) || null,
    id_usuario        : String(r['id_usuario'] || r['ID do jogador'] || ''),
