...
function GlobalLinkModal({ onClose }: { onClose: () => void }) {
  ...
  const handleCreate = async () => {
    if (!user || user.id === "guest") { toast.error("Conecte seu Telegram primeiro"); return; }
    if (!novoNome.trim() || !novoGravadora.trim()) { toast.error("Nome e gravadora são obrigatórios"); return; }
    setCreating(true);
    try {
      const res = await api.criarArtista({ nome: novoNome.trim(), foto: novoFoto.trim(), gravadora: novoGravadora.trim(), telegram_id: user.id });
      if (res.ok) {
        toast.success("Artista criado!", { description: `${novoNome} entrou no seu plantel.` });
        setNovoNome(""); setNovoFoto(""); setNovoGravadora("");
        onClose();
      } else { toast.error(res.erro || "Falha ao criar"); }
    } catch { toast.error("Erro na conexão"); }
    finally { setCreating(false); }
  };

  return (
    <div className="fixed inset-0 z-[110] flex items-center justify-center p-4 bg-black/80 backdrop-blur-md">
      <motion.div initial={{ y: 20, opacity: 0 }} animate={{ y: 0, opacity: 1 }} className="w-full max-w-sm bg-card border border-white/10 rounded-[3rem] p-6 shadow-2xl relative max-h-[85vh] flex flex-col">
        ...
        <div className="grid grid-cols-2 gap-2 p-1 bg-white/[0.03] border border-white/5 rounded-2xl mb-4">
          <button onClick={() => setTab("link")} className={`py-2.5 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all ${tab === "link" ? "bg-primary text-primary-foreground shadow-lg" : "text-muted-foreground"}`}>Vincular</button>
          <button onClick={() => setTab("create")} className={`py-2.5 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all ${tab === "create" ? "bg-primary text-primary-foreground shadow-lg" : "text-muted-foreground"}`}>Criar Novo</button>
        </div>
        {tab === "link" ? (
          <>
            <div className="relative mb-3">
              <Search className="absolute left-3 top-1/2 -translate-y-1/2 size-4 text-muted-foreground" />
              <input value={q} onChange={e => setQ(e.target.value)} placeholder="Buscar artista..." className="w-full h-11 bg-white/5 border border-white/10 rounded-2xl pl-10 pr-4 text-sm font-bold uppercase tracking-tighter outline-none focus:border-primary/40" />
            </div>
            <div className="flex-1 overflow-y-auto space-y-2 pr-1 scrollbar-hide py-1 min-h-[200px]">
              {available === null ? (
                <div className="space-y-2">{Array.from({ length: 4 }).map((_, i) => <div key={i} className="h-14 rounded-2xl bg-white/[0.03] animate-pulse" />)}</div>
              ) : filtered.length === 0 ? (
                ...
                return (
                  <button key={a.nome} onClick={() => toggle(a.nome)} className={`w-full flex items-center gap-3 p-3 rounded-2xl border transition-all text-left ${isSel ? "bg-primary/15 border-primary/40" : "bg-white/[0.03] border-white/5 hover:bg-white/[0.06]"}`}>
                    <div className={`size-5 rounded-md border-2 flex-shrink-0 grid place-items-center transition-all ${isSel ? "bg-primary border-primary" : "border-white/20"}`}>{isSel && <span className="text-[10px] font-black text-primary-foreground">✓</span>}</div>
            ...
            <button disabled={selected.size === 0 || linking} onClick={handleLinkAll} className="mt-4 w-full h-14 rounded-[2rem] bg-primary text-primary-foreground font-black uppercase text-[11px] tracking-[0.2em] active:scale-95 transition-all disabled:opacity-30 disabled:cursor-not-allowed shadow-[0_10px_30px_rgba(var(--primary-rgb),0.3)]">
              {linking ? "Vinculando..." : selected.size === 0 ? "Selecione ao menos um" : `Vincular ${selected.size} artista${selected.size > 1 ? "s" : ""}`}
            </button>
          </>
        ) : (
          <div className="flex-1 overflow-y-auto pr-1 space-y-3">
            <div><label className="text-[10px] font-black uppercase tracking-widest text-muted-foreground mb-1.5 block">Nome do artista *</label><input value={novoNome} onChange={e => setNovoNome(e.target.value)} placeholder="Ex: Lana Empire" className="w-full h-12 bg-white/5 border border-white/10 rounded-2xl px-4 text-sm font-bold outline-none focus:border-primary/40" /></div>
            <div><label className="text-[10px] font-black uppercase tracking-widest text-muted-foreground mb-1.5 block">Foto (link Google Drive)</label><input value={novoFoto} onChange={e => setNovoFoto(e.target.value)} placeholder="https://drive.google.com/file/d/..." className="w-full h-12 bg-white/5 border border-white/10 rounded-2xl px-4 text-xs font-bold outline-none focus:border-primary/40" /><p className="text-[9px] text-muted-foreground/60 mt-1 px-1">Cole o link de compartilhamento do Drive. Opcional.</p></div>
            <div><label className="text-[10px] font-black uppercase tracking-widest text-muted-foreground mb-1.5 block">Gravadora *</label><input value={novoGravadora} onChange={e => setNovoGravadora(e.target.value)} placeholder="Ex: Independent, Empire Records..." className="w-full h-12 bg-white/5 border border-white/10 rounded-2xl px-4 text-sm font-bold outline-none focus:border-primary/40" /></div>
            <button disabled={creating || !novoNome.trim() || !novoGravadora.trim()} onClick={handleCreate} className="mt-2 w-full h-14 rounded-[2rem] bg-primary text-primary-foreground font-black uppercase text-[11px] tracking-[0.2em] active:scale-95 transition-all disabled:opacity-30 shadow-[0_10px_30px_rgba(var(--primary-rgb),0.3)]">
...
function BottomNav() {
  ...
  return (
    <nav className="fixed bottom-0 inset-x-0 z-40 border-t border-white/5 bg-background/90 backdrop-blur-xl" role="navigation" aria-label="Navegação principal">
      <div className="mx-auto max-w-md flex items-stretch justify-around px-1 pt-1.5 pb-[max(0.5rem,env(safe-area-inset-bottom))]">
        {items.map((it) => {
          ...
          const Icon = it.icon;
          return (
            <Link key={it.to} to={it.to} search={it.search} preload="intent" onClick={() => haptic.selection()} aria-label={it.label} aria-current={active ? "page" : undefined} className={`relative flex flex-col items-center justify-center gap-0.5 min-h-11 flex-1 rounded-2xl transition-colors ${active ? "text-primary" : "text-muted-foreground"}`}>
              ...
              <span className={`text-[9px] font-bold uppercase tracking-tight ${active ? "opacity-100" : "opacity-80"}`}>{it.label}</span>
            </Link>
          );
        })}
      </div>
...
function RootInner() {
  ...
  useEffect(() => {
    ...
    if (!w) return;
    try { w.setHeaderColor?.("#000000"); w.setBackgroundColor?.("#000000"); } catch {}
  }, []);

  const isHome = pathname === "/";
  const handleBack = () => { haptic.light(); router.history.back(); };
  ...
  return (
    <div className="min-h-screen flex flex-col bg-background pb-24" style={{ paddingTop: "calc(4rem + env(safe-area-inset-top))" }}>
      <nav className="fixed top-0 inset-x-0 z-[60] bg-background/80 backdrop-blur-xl border-b border-white/5 flex items-center justify-between px-6 h-16 pt-[env(safe-area-inset-top)]" style={{ height: "calc(4rem + env(safe-area-inset-top))" }}>
        ...
        <button onClick={() => { haptic.light(); setIsOpen(!isOpen); }} aria-label={isOpen ? "Fechar menu" : "Abrir menu"} aria-expanded={isOpen} className="min-h-11 min-w-11 -mr-2 grid place-items-center text-foreground active:scale-95 transition-transform">
      ...
      <AnimatePresence>
        {isOpen && (
          <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: 20 }} className="fixed inset-0 z-50 bg-background pt-20 px-6 overflow-y-auto">
            <div className="space-y-4 pb-12">
              ...
              <Link to="/tv" onClick={() => setIsOpen(false)} className="flex items-center gap-4 p-5 rounded-3xl bg-red-600/10 border border-red-600/30 text-red-500 hover:bg-red-600/15 transition-all">
