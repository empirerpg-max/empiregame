...
function PontoPlaylistsPlanilha() {
  ...
  const carregarDados = useCallback(async () => {
    ...
    try {
      const [pts, sal]: [any, any] = await Promise.all([
        fetchData({ acao: "ponto_listar_pontos", tgId }).catch((err) => ({ erro: err.message })),
    ...
    for (const [plataforma, playlist] of Object.entries(selecoes)) {
      try {
        ...
        if (d?.ok) {
          ...
          erros.push(`${plataforma}: ${d?.erro || "Erro desconhecido"}`);
        }
      } catch(err: any) {
        erros.push(`${plataforma}: ${err.message}`);
      }
    }
  ...
  return (
    <main className="flex-1 mx-auto w-full max-w-md px-6 pt-6 pb-24 flex flex-col gap-4">
      ...
      {msg?.key === "global" && (
        <div className="p-4 bg-red-950/40 border border-red-500/50 rounded-2xl text-red-400 text-sm font-semibold leading-relaxed mb-2">
          🚨 Atenção: <br/>{msg.text}
        </div>
      )}
