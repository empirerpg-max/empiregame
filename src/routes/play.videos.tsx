...
export default function PlayVideos() {
  ...
  useEffect(() => {
    async function load() {
      console.log('[Play/Videos] Iniciando busca no Supabase…');
      setLoading(true);
      setError(null);
      ...
      if (sbError) {
        console.error('[Play/Videos] Erro Supabase:', sbError);
        setError(`Erro ao carregar vídeos: ${sbError.message}`);
        setLoading(false);
        return;
      }
      ...
      if (!data || data.length === 0) {
        console.warn(
          '[Play/Videos] Supabase retornou array vazio. ' +
            'Verifique RLS na tabela Videos ou se há registros cadastrados.'
        );
        setItems([]);
        setLoading(false);
        return;
      ...
      console.log(`[Play/Videos] ${data.length} registros recebidos do Supabase:`, data);

      // Coleta todos os telegram_file_ids para resolução em lote
      const fileIds = data
        .map((row: any) => row.telegram_file_id || '')
        .filter(Boolean) as string[];

      console.log(`[Play/Videos] Resolvendo ${fileIds.length} telegram_file_ids…`);
      ...
      console.log('[Play/Videos] Items mapeados (com streamUrls):', mapped);
      setItems(mapped);
      setLoading(false);
    }
  ...
  if (loading)
    return (
      <div className="p-4 pb-32">
        ...
        <div className="text-center mt-10 text-gray-400">Carregando vídeos…</div>
      </div>
    );
      ...
      <div className="p-4 pb-32">
        ...
        <div className="text-center mt-10 text-gray-400">
          ...
          <span className="text-xs text-gray-500">
            (Verifique RLS na tabela <code>Videos</code> no Supabase)
          </span>
        </div>
      </div>
    );
          ...
          >
            ...
            {item.streamUrl ? (
              <video
                ...
                onError={(e) =>
                  console.error('[Play/Videos] Erro ao reproduzir vídeo:', item.titulo, e)
                }
              />
            ) : item.capa ? (
              <img
