...
export default function PlayMusicas() {
  const [items, setItems] = useState<MusicaItem[]>([]);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState<string | null>(null);
  const { setNowPlaying } = usePlay();

  useEffect(() => {
    console.log('[Play/Musicas] Iniciando busca no Supabase…');
    ...
    supabase
      ...
      .then(({ data, error: sbError }) => {
        if (sbError) {
          console.error('[Play/Musicas] Erro Supabase:', sbError);
          setError(`Erro ao carregar músicas: ${sbError.message}`);
          return;
        }

        if (!data || data.length === 0) {
          console.warn(
            '[Play/Musicas] Supabase retornou array vazio. ' +
              'Verifique RLS na tabela Musicas ou se há registros cadastrados.'
          );
          setItems([]);
          return;
        }

        console.log(`[Play/Musicas] ${data.length} registros recebidos:`, data);

        const mapped: MusicaItem[] = data.map((row: any) => ({
        ...
        console.log('[Play/Musicas] Items mapeados:', mapped);
        setItems(mapped);
      })
  ...
  if (loading)
    return (
      <div className="p-4 pb-32">
        ...
        <div className="text-center mt-10 text-gray-400">Carregando músicas…</div>
      </div>
    );
      ...
      <div className="p-4 pb-32">
        ...
        <div className="text-center mt-10 text-gray-400">
          ...
          <span className="text-xs text-gray-500">
            (Verifique RLS na tabela <code>Musicas</code> no Supabase)
          </span>
        </div>
      </div>
    );
