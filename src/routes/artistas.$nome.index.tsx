...
function ArtistDashboard() {
  ...
  useEffect(() => {
    ...
    Promise.all([
      api.meusArtistas(user.id).catch(() => []),
      api.listarAlbuns(nome).catch(() => []),
      api.listTours().catch(() => []),
    ]).then(([list, albunsList, toursList]) => {
      // 1. Seta o Artista
      const art = list.find((a) => a.nome?.trim().toLowerCase() === safeNome) || null;
      setArtist(art);
      setAlbuns(albunsList);

      // 2. Busca e Seta os dados da Turnê (se existir)
      const tList = toursList.find((t) => t.artista?.trim().toLowerCase() === safeNome);
    ...
    <main className="flex-1 pb-24 bg-background">
          ...
          <div className="flex items-end gap-4">
            ...
            <div className="flex-1 min-w-0 pb-1">
              ...
              <div className="flex items-center gap-2">
                <div
                  className={`size-1.5 rounded-full ${artist.status === "Livre" ? "bg-primary animate-pulse" : "bg-yellow-500"}`}
                />
      ...
      <div className="px-6 space-y-8 relative z-20 -mt-2">
        ...
        <div className="grid grid-cols-3 gap-3">
          ...
          />
          <QuickAction
            icon={<Mic2 className="size-6 text-emerald-400" />}
            label="Turnê"
            id="btn-tour"
            to="/acoes/tour"
            params={{ nome: artist.nome }}
          />
          <QuickAction
            icon={<Film className="size-6 text-blue-400" />}
            label="Cinema"
            id="btn-cinema"
            to="/acoes/cinema"
            params={{ nome: artist.nome }}
        ...
        <section>
          <div className="flex gap-2 p-1 bg-card rounded-[2rem] border border-white/5 mb-6">
            <TabButton active={activeTab === "entretenimento"} onClick={() => setActiveTab("entretenimento")}>
              Gestão
            </TabButton>
            <TabButton active={activeTab === "bens"} onClick={() => setActiveTab("bens")}>
              Bens
            </TabButton>
            ...
            {activeTab === "entretenimento" && (
              <div className="grid grid-cols-2 gap-3">
                ...
                <div className="col-span-2 mt-4 space-y-3">
                  <h4 className="text-[10px] font-black uppercase tracking-widest text-muted-foreground/30 px-1">
                    Administrativo
                  </h4>
                  <div className="grid grid-cols-2 gap-3">
                    <MiniAction
                      ...
                      color="text-destructive font-black"
                    />
                  </div>
                </div>
              </div>
            ...
            {activeTab === "bens" && (
              <div className="space-y-4">
                <div className="flex items-center justify-between px-1">
                  <h4 className="text-[10px] font-black uppercase tracking-widest text-muted-foreground/50 text-emerald-500">
                    Patrimônio Adquirido
                  </h4>
                  <button onClick={() => setModal("imovel")} className="text-[10px] font-black text-primary uppercase">
                    + Comprar
                  </button>
                </div>
                <Link
                  ...
                  params={{ nome: artist.nome }}
                  className="block p-8 rounded-[2.5rem] bg-card border border-white/5 text-center transition-all hover:bg-white/[0.04]"
                >
                  <Building2 className="size-10 text-muted-foreground/20 mx-auto mb-4" />
                  <p className="text-xs text-muted-foreground italic">
                    Clique para ver inventário de imóveis e empresas.
                  </p>
                </Link>
              </div>
            )}
            ...
            {activeTab === "tours" && (
              <div className="space-y-4">
                <div className="flex items-center justify-between px-1">
                  ...
                  <Link to="/tours" className="text-[10px] font-black text-primary uppercase">
                    Todas Turnês
                  </Link>
                </div>

                {/* ✅ Lógica de Turnê adicionada com sucesso aqui! */}
                {tourData ? (
                  <Link
                    to="/tours/$nome"
                    params={{ nome: artist.nome }}
                    className="block p-5 rounded-[2.5rem] bg-card border border-white/5 transition-all hover:bg-white/[0.04] hover:scale-[1.01] group relative overflow-hidden shadow-lg"
                  >
                    <div className="absolute -right-10 -bottom-10 size-32 bg-primary/10 blur-[40px] rounded-full opacity-0 group-hover:opacity-100 transition-opacity duration-500" />
                    <div className="flex items-center gap-4 relative z-10">
                      <div className="size-12 rounded-2xl bg-primary/20 text-primary grid place-items-center shrink-0">
                        <Mic2 className="size-6" />
                      </div>
                      <div className="flex-1 min-w-0">
                        <h4 className="text-base font-black italic uppercase tracking-tighter truncate">
                          {tourData.titulo}
                        </h4>
                        <p className="text-[10px] font-bold text-muted-foreground uppercase tracking-widest mt-0.5">
                          {tourData.realizados} / {tourData.total} Shows •{" "}
                          <span className="text-primary">{tourData.status}</span>
                        </p>
                      </div>
        ...
        <section className="pt-4 border-t border-white/5">
          <div className="flex items-center justify-between mb-5 px-1">
            ...
            <Link
              ...
              className="size-10 rounded-xl bg-primary/10 text-primary grid place-items-center active:scale-90 transition-transform"
            >
            ...
            <div className="grid grid-cols-3 gap-4">
              {albuns.map((a) => (
                <Link key={a.id} to="/album/$id" params={{ id: a.id! }} className="group">
                  ...
                  <p className="mt-2 text-[10px] font-black uppercase tracking-tight text-center truncate">
                    {a.titulo}
                  </p>
                </Link>
              ))}
...
function StatCardV2({ label, value, icon, accent }: any) {
  return (
    <div
      className={`p-4 rounded-[1.5rem] border transition-all duration-300 shadow-lg ${
    ...
    >
      <div className="flex items-center gap-2 mb-2 text-muted-foreground/70">
        <div
          className={`size-7 rounded-lg grid place-items-center ${accent ? "bg-primary/20 text-primary" : "bg-white/10 text-white/50"}`}
        >
          {icon}
        </div>
      ...
      <p
        className={`text-base font-black italic tracking-tighter truncate ${accent ? "text-primary" : "text-foreground"}`}
      >
        {value}
      </p>
    </div>
...
function StatCompact({ label, value, max, icon, color, reverse }: any) {
  ...
  return (
    <div className="p-4 rounded-[1.8rem] bg-white/5 backdrop-blur-md border border-white/10 flex items-center gap-4">
      <div className={`size-10 rounded-xl bg-white/5 grid place-items-center shadow-inner shrink-0 ${color}`}>
        {icon}
      </div>
      <div className="flex-1 min-w-0">
        <div className="flex justify-between items-end mb-2">
          <span className="text-[11px] font-black uppercase tracking-widest text-muted-foreground/60 truncate mr-1">
            {label}
          </span>
          <span className="text-xs font-black tracking-tighter shrink-0">
...
function QuickAction({ icon, label, to, params, id }: any) {
  return (
    ...
    >
      ...
      <span className="text-[10px] font-black uppercase tracking-[0.2em] text-muted-foreground group-hover:text-foreground text-center leading-none">
        {label}
      </span>
    </Link>
  );
...
function MiniAction({ label, icon, onClick, to, color }: any) {
  ...
  const cls =
    "flex items-center gap-3 p-3 rounded-2xl bg-white/[0.02] border border-white/5 hover:bg-white/[0.05] transition-all active:scale-[0.98]";
    ...
    >
      ...
      >
        <div className="flex items-center justify-between mb-4">
          ...
          <button onClick={onClose} className="size-8 rounded-full bg-secondary grid place-items-center">
            <X className="size-4" />
          </button>
        </div>
        {children}
