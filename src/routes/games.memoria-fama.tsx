...
import { Brain, ChevronLeft, Loader2, Play, Wallet } from "lucide-react";
import { useTelegramUser } from "@/lib/telegram";
import { api, fmtEC, type Artist } from "@/lib/api";

export const Route = createFileRoute("/games/memoria-fama")({
  component: MemoriaFamaPage,
});

function MemoriaFamaPage() {
  ...
  if (launched && artist && user) {
    const src = `/games/memoria-fama/index.html?tg=${encodeURIComponent(user.id)}&artist=${encodeURIComponent(artist.nome)}`;
    return (
    ...
    <main className="flex-1 mx-auto w-full max-w-2xl px-4 pt-6 pb-32">
      ...
      <header className="mb-8">
        ...
        <h1 className="text-3xl font-black italic uppercase tracking-tighter">
          Memória da <span className="text-primary">Fama</span>
        </h1>
        <p className="text-muted-foreground text-[11px] font-bold mt-1 uppercase tracking-widest">
          Grade 4×4 · 60s · 30 E$C de entrada · até 3× de prêmio
        </p>
      </header>
      ...
      >
        <Play className="size-5 fill-current" /> Iniciar memória
      </button>

      <p className="text-[11px] text-muted-foreground/70 text-center mt-4 leading-relaxed">
        Entrada de 30 E$C debitada ao iniciar. Cards mostram seus artistas + um
        ícone pixel art. Sem erros = <span className="font-black">FLAWLESS</span> (+50%).
      </p>
    </main>
  );
}
